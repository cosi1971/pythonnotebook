...
importfile = r'C:\Users\leeko\Google Drive\Python notebook\expense1.xlsx'
...
timecol, seriescol= 'Date','Balance'
...
indexedts.min()[0], indexedts.mean()[0]
...
if indexedts.min()[0] <0:
    indexedtspos=indexedts-indexedts.min()+1
    indexedtspos.min()
else:
...
test_stationary(indexedtslog,winperiod)


# In[154]:


MA=indexedtslog.rolling(window=winperiod).mean()
Mstd=indexedtslog.rolling(window=winperiod).std()
indexedtslogminusMA=indexedtslog - MA
indexedtslogminusMA.dropna(inplace=True)
test_stationary(indexedtslogminusMA,winperiod)


# In[155]:


expdecwAvg=indexedtslog.ewm(halflife=winperiod,min_periods=0,adjust=True).mean()
plt.plot(indexedtslog, label=seriescol)
...
indexedtslogminusEMA=indexedtslog-expdecwAvg
test_stationary(indexedtslogminusEMA,winperiod)
...
indexedtslogdiffshift=indexedtslog-indexedtslog.shift()
indexedtslogdiffshift.dropna(inplace=True)
plt.plot(indexedtslogdiffshift)
test_stationary(indexedtslogdiffshift,winperiod)
...
timeseries=indexedtslogdiffshift
...
pred_ARIMA=np.exp(pred_ARIMA_log)-1+indexedts[seriescol].min()
# pred_ARIMA=pred_ARIMA_log
plt.plot(indexedtspos, color='blue')
